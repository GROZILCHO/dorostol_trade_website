...
Runtime header and footer logos use optimized WebP files from `dorostol-trade-website/public/assets/brand/`. The original logo PNG files remain in the same directory as source/reference backups. Below-fold map, product, and footer-logo images use lazy loading with asynchronous decoding. An optimized `dorostol-mark-runtime.png` variant is available for a future favicon switch, but the current favicon path remains unchanged. Moving oversized original PNG backups outside `public/` remains an optional deployment-artifact cleanup after launch-package testing.
